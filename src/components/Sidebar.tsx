      ...
      <aside className={`print:hidden fixed lg:sticky top-0 left-0 z-[100] h-screen border-r border-[var(--border)] bg-[var(--background)] p-4 flex flex-col transform transition-all duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} ${isCollapsed ? 'lg:w-20 w-64' : 'w-64'} text-[14px]`}>
        ...
        <nav className="flex flex-col gap-1.5 text-[var(--muted)] font-bold flex-1 overflow-y-auto overflow-x-hidden no-scrollbar">
          ...
          <div className="mt-8 flex flex-col gap-1.5 font-bold">
            ...
            <Link href="/settings?tab=profile" onClick={onClose} className={`py-2 px-3 rounded-lg hover:bg-[var(--hover-bg)] hover:text-[var(--foreground)] transition-all flex items-center gap-2.5 cursor-pointer hover:scale-[1.02] ${pathname === '/settings' && typeof window !== 'undefined' && window.location.search.includes('tab=profile') ? 'bg-[var(--hover-bg)] text-[var(--foreground)] shadow-sm' : ''} ${isCollapsed ? 'lg:justify-center' : ''}`}>
              <Users className="w-4 h-4 shrink-0" />
              <span className={`truncate ${isCollapsed ? 'lg:hidden' : ''}`}>{t('settings.tabProfile') || 'Profile'}</span>
            </Link>
          </div>
        </nav>
